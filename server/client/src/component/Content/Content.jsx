...
const Content = ({ match: { params }, location: { state } }) => {
    ...
    useEffect(() => {
        ...
        room = room.substr(1);
        socket = io(ENDPOINT);
        socket.emit('createRoom', { name: params.username, room: room, url: state }, (error) => {
            if (error) {
                console.log(error)
            }
        });
        socket.emit('getUrlEvent', (error) => {
            if (error) {
                alert(error);
            }
        })
        ...
        <section id="content">
            ...
            loaded
                ...
                <div className="content-video-controls-options">
                    ...
                    <input className="range" type='range' min={0} max={1} step='any' value={volume} onChange={handleVolumeChange} />
                </div>
            </div>
